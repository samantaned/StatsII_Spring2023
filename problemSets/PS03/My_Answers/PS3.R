...
coef_table <- coef(multinom_model1)
se_table <- sqrt(diag(vcov(multinom_model1)))

# Combine coefficient and standard error tables
output_table <- cbind(coef_table, se_table)

# Create a table object
tab <- xtable(output_table)

# Print the table in LaTeX format
print(tab, include.rownames = TRUE, booktabs = TRUE)
