...
graduation$hsgrad <- ifelse(graduation$hsgrad == "yes", 0, 1) #1 = has graduated
graduation$nonwhite <- ifelse(graduation$nonwhite == "yes", 0, 1) #1 = no, not nonwhite
mylogit <- glm(hsgrad ~ nonwhite + mhs + fhs + income + asvab + nsibs + intact, data = graduation, family = "binomial")
